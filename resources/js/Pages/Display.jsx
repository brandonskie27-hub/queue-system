    ...
    return (
        <div className="flex min-h-screen flex-col bg-brand-50 text-gray-900">
            ...
            <main className="flex flex-1 flex-col gap-8 p-8">
                <section
                    className={`rounded-2xl border-l-8 p-8 text-center shadow-sm transition-colors duration-700 ${
                        isFlashing
                            ? 'animate-pulse border-gold-600 bg-gold-200'
                            : 'border-brand-700 bg-white'
                    }`}
                >
                    {lastCalled ? (
                        <>
                            <p className="text-2xl font-semibold uppercase tracking-widest text-brand-700">
                                {lastCalled.service}
                            </p>
                            <p className="my-2 text-9xl font-black tabular-nums text-brand-900">
                                {lastCalled.code}
                            </p>
                            <p className="text-5xl font-semibold text-brand-800">
                                Please proceed to {lastCalled.counter}
                            </p>
                        </>
                    ) : (
                        <p className="py-12 text-4xl text-gray-500">
                                ...
                                {service.serving.length === 0 ? (
                                    ...
                                    <ul className="divide-y divide-brand-100">
                                        {service.serving.map((ticket) => (
                                            ...
                                            >
                                                <span className="text-5xl font-bold tabular-nums text-brand-900">
                                                    {ticket.code}
                                                </span>
